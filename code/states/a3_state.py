...
from states.tag_generation_state import generate_tag_types_prompt
from consts import (
    MANAGER,
    LLM_TAGS_GENERATOR,
    TAG_TYPE_ASSIGNER,
...
def initialize_a3_state(config: dict, input_text: str = None) -> A3SystemState:
    """
    Initialize the A3 system state from config.

    Args:
        config: A3 system configuration dictionary
        input_text: Optional input text (can be None for template creation)

    Returns:
        A3SystemState with all values initialized
    """
    ...
    tag_types_prompt = generate_tag_types_prompt(config["tag_types"])
    ...
    references_gen_messages = [
        SystemMessage(
            build_system_prompt_message(agents[REFERENCES_GENERATOR]["prompt_config"])
        ),
        SystemMessage(
    ...
    references_selector_messages = [
        SystemMessage(
            build_system_prompt_message(agents[REFERENCES_SELECTOR]["prompt_config"])
        ),
        SystemMessage(
