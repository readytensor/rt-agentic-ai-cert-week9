...
def print_a3_response(response: Dict[str, Any], title: str = "A3-SYSTEM DEMO", use_rich: bool = True) -> Dict[str, Any]:
    """
...
def _print_a3_response_rich(response: Dict[str, Any], title: str) -> Dict[str, Any]:
    ...
    console.print(Panel(manager_brief, title="[bold green]📋 Manager Brief[/bold green]", border_style="green"))
    
    # Title
    title_content = response.get("title", "(Not generated)")
    console.print(Panel(title_content, title="[bold yellow]📝 Title[/bold yellow]", border_style="yellow"))
    
    # TL;DR
    tldr_content = response.get("tldr", "(Not generated)")
    console.print(Panel(tldr_content, title="[bold cyan]📄 TL;DR[/bold cyan]", border_style="cyan"))
    ...
    if selected_tags:
        ...
        console.print(Panel(tags_text, title="[bold magenta]🔖 Tags[/bold magenta]", border_style="magenta"))
    else:
        console.print(Panel("(No tags generated)", title="[bold magenta]🔖 Tags[/bold magenta]", border_style="magenta"))
    
    # Search queries
    queries = response.get("reference_search_queries", [])
    if queries:
        queries_text = "\n".join(f"• {query}" for query in queries)
        console.print(Panel(queries_text, title="[bold blue]🔍 Search Queries[/bold blue]", border_style="blue"))
    else:
        console.print(Panel("(No search queries generated)", title="[bold blue]🔍 Search Queries[/bold blue]", border_style="blue"))
    ...
    console.print(Panel(refs_content, title="[bold red]📚 References[/bold red]", border_style="red"))
...
def print_a3_response_compact(response: Dict[str, Any], use_rich: bool = True) -> Dict[str, Any]:
    """
    ...
    if use_rich and RICH_AVAILABLE:
        ...
        selected_references = response.get("selected_references", [])
        if selected_references:
            console.print(f"[red]📚 References:[/red] {len(selected_references)} selected")
