...
def test_title_generator_node_returns_cleaned_title(monkeypatch, title_generator_state):
    ...
    assert (
        result[TITLE] == "Machine Learning in Healthcare: A Comprehensive Review"
    )  # Stripped
...
def test_title_generator_node_skips_when_already_approved(
    monkeypatch, title_generator_state
):
...
def test_title_generator_node_handles_empty_title_response(
    monkeypatch, title_generator_state
):
...
def test_title_generator_node_handles_none_content_response(
    monkeypatch, title_generator_state
):
...
def test_title_generator_node_constructs_correct_message_sequence(
    monkeypatch, title_generator_state
):
...
def test_title_generator_node_propagates_llm_exceptions(
    monkeypatch, title_generator_state
):
...
def test_title_generator_node_passes_correct_feedback_type(
    monkeypatch, title_generator_state
):
    ...
    mock__get_reviewer_message.assert_called_once_with(
        title_generator_state, TITLE_FEEDBACK
    )
    assert result[TITLE] == "Generated Title"
